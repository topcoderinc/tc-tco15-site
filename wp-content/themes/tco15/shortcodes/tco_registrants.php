	...
	if ($response ['response'] ['code'] == 200) {
		if (substr ( $response ['body'], 0, 5 ) != "ERROR") {
			...
			$result = '<table class="table">
							<thead>
								<th>Handle</th>
								<th class="text-right">Total: '. number_format(count($xml->children())) .'</th>
							</thead>
							<tbody>';
			foreach ( $xml->children() as $value ) {
				$result .= '	<tr>
									<td colspan="2"><a href="http://www.topcoder.com/tc?module=MemberProfile&cr='.$value->user_id.'" style="color:'.$value->handlecolor.'">'.$value->handle.'</a></td>
								</tr>';
			}
			$result .= '	</tbody>
						</table>';
